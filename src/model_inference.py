...
class MST:
    ...
    def inference(self, path, mask_path, valid_th, mask_th, output_path=None, not_obj_remove=False):
        ...
        # origin_img=cv2.resize(origin_img,(self.img_size,self.img_size), interpolation=cv2.INTER_NEAREST)
        #
        # # origin_img=Image.open(path)
        # # origin_img=origin_img.resize((self.img_size, self.img_size))
        # index=origin_img[:,:,0]>220
        # #
        # # origin_img[origin_img[:, :, 0] > 220]=0
        # mask=np.ones((self.img_size,self.img_size))*index
        #
        # plt.imshow(mask)
        # plt.show()
        # items['masks'] = torchvision.transforms.functional.to_tensor(mask.astype('float32'))
        for k in items:
